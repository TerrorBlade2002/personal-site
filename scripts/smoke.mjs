...
check('archive shows a clear call to action', await page.$eval('.placeholder-cta', (b) => b.textContent.trim() === 'Show projects'))
check('archive shows placeholder tiles', (await page.$$eval('.placeholder', (n) => n.length)) === 3)
...
const banned = /nothing is shown|shown proactively|declassif|REDACTED|hands-on museum|this site is|physics lab|honest-abstraction|award.winning|treat yourself|uncharted|paged for/i
const homeText = await page.evaluate(() => document.body.innerText)
check('home copy has no meta-narration', !banned.test(homeText))
check('hero headline is the production line', (await page.$eval('.hero-title', (h) => h.textContent)).includes('survive production'))
check('no LAB label rendered', await page.evaluate(() => !/LAB/.test(document.body.innerText)))
...
await go('nope')
await wait(500)
check('404 route renders', await page.$eval('.empty-state', (e) => e.textContent.includes('No such path')))
